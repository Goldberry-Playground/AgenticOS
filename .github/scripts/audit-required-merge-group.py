...
GOL-1735 / GOL-1406-D. GitHub's merge queue builds a synthetic `merge_group`
commit and waits for every *required* status check to report on it. A required
check whose workflow does not trigger on `merge_group` never reports there, so
the queue entry sits at "Expected — waiting for status" until it times out and
is dropped — the whole queue wedges. This audit makes that failure mode a
loud, pre-merge error instead of a silent production wedge.

Two modes:

  static (default)  Parse .github/workflows/*. y[a]ml and .github/required-checks.json.
                    For each declared required context, find the workflow job(s)
                    that produce it and assert at least one such workflow lists
...
def static_audit():
    ...
    for ctx in required:
        ...
        if not producers:
            ...
            hint = (f" (workflows with templated job names that may produce it: "
                    f"{sorted(set(templated))})") if templated else ""
            failures.append(f"required check '{ctx}' is produced by NO workflow "
                            f"job — renamed or removed?{hint}")
